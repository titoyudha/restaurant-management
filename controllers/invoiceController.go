...
var (
	invoiceCollection *mongo.Collection = databases.OpenCollection(databases.Client, "invoice")
)
...
func GetInvoiceByID() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if invoiceView.Payment_method != "null" {
...
func CreateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		invoice.Update_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
...
func UpdateInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		invoice.Update_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
		updateObj = append(updateObj, bson.E{"updated at", invoice.Update_at})
		...
		if err != nil {
			msg := fmt.Sprintf("invoice updated failed")
			c.JSON(http.StatusInternalServerError, gin.H{"error ": msg})
			return
		}
		c.JSON(http.StatusOK, result)
